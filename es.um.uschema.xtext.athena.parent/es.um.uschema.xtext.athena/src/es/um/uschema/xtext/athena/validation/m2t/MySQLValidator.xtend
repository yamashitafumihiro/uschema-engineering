...
class MySQLValidator
{
  ...
  def validate(AthenaSchema schema)
  {
    ...
    if (EcoreUtil2.getAllContentsOfType(schema, SimpleReferenceTarget).exists[r | handler.isMultiplicityToMany(r.multiplicity)])
    ...
    if (EcoreUtil2.getAllContentsOfType(schema, SimpleAggregateTarget).exists[a | handler.isMultiplicityToMany(a.multiplicity)])
  ...
  private def dispatch checkDataType(DataType theType)
  {
    if (theType === null)
    {
      ...
      switch (theType)
      {
        ...
        {
          warningLog.add("SimpleFeature - In MySQL a column of type Null cannot be created. Replacing by String: " + athenaIO.serialize(theType.eContainer) + "\n")
          return factory.createUnrestrictedPrimitiveType("String")
        }
        case theType instanceof Map:
        {
          ...
          {
            warningLog.add("SimpleFeature - In MySQL a Map cannot be of Null values. Replacing by String: " + athenaIO.serialize(theType.eContainer) + "\n")
