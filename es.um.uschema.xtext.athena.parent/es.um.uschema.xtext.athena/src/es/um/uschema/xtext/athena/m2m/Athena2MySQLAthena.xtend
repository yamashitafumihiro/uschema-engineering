...
class Athena2MySQLAthena
{
  ...
  def AthenaSchema m2m(AthenaSchema schema, boolean mysqlNormalizeRelations, boolean mysqlNormalizeCollections)
  {
    if (mysqlNormalizeRelations)
    {
      ...
      for (ref : EcoreUtil2.getAllContentsOfType(schema, SimpleReferenceTarget).filter[r | handler.isMultiplicityToMany(r.multiplicity)])
      {
        ...
        if (keyFeat !== null && keyFeat.type !== null && keyFeat.type instanceof SinglePrimitiveType)
          ref.type = EcoreUtil2.copy(keyFeat.type) as SinglePrimitiveType
      }
  
      // Transform each Aggregate 1..* by moving it to target Entity and letting it reference the original Entity container.
      for (aggr : EcoreUtil2.getAllContentsOfType(schema, SimpleAggregateTarget).filter[a | handler.isMultiplicityToMany(a.multiplicity)])
      {
