...
import es.um.uschema.xtext.athena.athena.ComposedReferenceTarget
import es.um.uschema.xtext.athena.athena.SinglePrimitiveType
import es.um.uschema.xtext.athena.utils.compare.ComparePrimitiveType
...
import es.um.uschema.xtext.athena.athena.TopLevelStructureDefiningElementDecl
...
class AthenaWarningValidator extends AbstractAthenaValidator
{
  ...
  def checkNoEntityCommonWOptionals(RegularEntityDecl entity)
  {
    ...
    for (SimpleFeature sFeat : fSet.features.filter(SimpleFeature).filter[feat | feat.optional])
      warning("Entity \"" + entity.name + "\" should not contain optional features in a Common section: \"" + sFeat.name + "\"", Literals.TOP_LEVEL_STRUCTURE_DEFINING_ELEMENT_DECL__NAME)
  }
