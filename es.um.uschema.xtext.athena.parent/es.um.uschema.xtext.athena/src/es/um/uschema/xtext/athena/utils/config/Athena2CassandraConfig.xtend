package es.um.uschema.xtext.athena.utils.config
...
import es.um.uschema.xtext.athena.utils.config.defaults.DefaultConfig

class Athena2CassandraConfig
{
  Path inputModel
  Path inputPath
  Path outputPath
  ...
  new()
  {
    ...
    this.inputPath = null
    this.outputPath = Path.of(defaultProperties.getProperty("folder_athena2cassandra"))
  }
  ...
  new(String[] args)
  {
    ...
    try
    {
      ...
      if (cmd.hasOption("h"))
      {
        formatter.printHelp("<Athena2Cassandra Main>", options)
        System.exit(0)
      }
