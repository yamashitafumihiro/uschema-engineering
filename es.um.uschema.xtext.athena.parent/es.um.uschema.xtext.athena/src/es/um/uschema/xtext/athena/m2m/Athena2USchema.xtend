...
class Athena2USchema
{
  ...
  def USchema m2m(AthenaSchema schema)
  {
    val nSchema = new AthenaNormalizer().m2m(schema)
    val newSchema = factory.createUSchema(nSchema.id.name)
  ...
  private def dispatch es.um.uschema.USchema.DataType transformDataType(PrimitiveType pType)
  {
  ...
  private def dispatch es.um.uschema.USchema.DataType transformDataType(Null aNull)
  {
    return factory.createNull
  }

  private def dispatch es.um.uschema.USchema.DataType transformDataType(List list)
  {
  ...
  private def dispatch es.um.uschema.USchema.DataType transformDataType(Set set)
  {
  ...
  private def dispatch es.um.uschema.USchema.DataType transformDataType(Tuple tuple)
  {
  ...
  private def dispatch es.um.uschema.USchema.DataType transformDataType(es.um.uschema.xtext.athena.athena.Map map)
  {
    val theKey = (map.keyType === null) ? factory.createPrimitiveType("String") : transformDataType(map.keyType) as es.um.uschema.USchema.PrimitiveType
  ...
  {
    if (multiplicity === null || handler.isMultiplicityToMany(multiplicity))
      return -1
    else if (handler.isMultiplicityToOne(multiplicity))
      return 1

    throw new IllegalArgumentException("Cannot get upper bound from multiplicity: " + multiplicity)
  }

  private def int getLowerBound(String multiplicity)
  {
    if (multiplicity === null || handler.isMultiplicityFromZero(multiplicity))
      return 0
    else if (handler.isMultiplicityFromOne(multiplicity))
      return 1

    throw new IllegalArgumentException("Cannot get lower bound from multiplicity: " + multiplicity)
  }
}
