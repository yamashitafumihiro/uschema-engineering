...
import es.um.uschema.subtypes.timestamp.TimestampInferrer;
import es.um.uschema.subtypes.timestamp.templates.TimestampAnalyzer;
import es.um.uschema.subtypes.util.configuration.TimestampInferenceConfig;

public class TimestampInferenceMain
{
  /**
  ...
  public static void runTimestampInference(TimestampInferenceConfig config)
  {
    ...
    USchema schema = new TimestampInferrer().infer(ip, dbName, analyzer, mrPath);

    outputPath.toFile().mkdirs();
    new EcoreModelIO().write(schema, outputPath.resolve(schema.getName() + ".xmi"));
  }
}
