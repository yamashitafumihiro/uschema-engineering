...
class GanttChart:
  ...
  def saveCharts(self, saveRoute):
    for key in self.__chartData:
      self.__saveChart(key, self.__chartData[key], saveRoute)

  def __saveChart(self, name, chartData, saveRoute):

    figure1 = pyplot.figure(figsize=(19,11),dpi=150)
    ...
    ax.xaxis.set_major_locator(RRuleLocator(rrulewrapper(MONTHLY, interval=6)))
...
#    pyplot.get_current_fig_manager().window.state('zoomed')
    pyplot.show()
#    figure1.savefig(saveRoute + name + ".svg")
